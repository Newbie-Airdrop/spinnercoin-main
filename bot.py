...
from colorama import *  # Tambahkan import colorama
...
def get_data(token):
    ...
    if response.status_code == 200:
        ...
        print(green + f"[ Nama ]: {name}" + reset)
        print(green + f"[ Saldo ]: {balance}" + reset)
        print(green + f"[ Level ]: {level}" + reset)  # Warna hijau untuk data yang berhasil diambil
        return data
    else:
        print(red + "Gaada tokennya ganteng" + reset)  # Warna merah jika gagal mengambil data
        return None
...
def click_spinner(token, repair):
    ...
    while True:
        ...
        # print(response.json())
        if response.status_code == 200:
            print(green + "[ Tap ]: Tapping..." + reset)
        else:
            print(green + "[ Tap ]: Tapping selesai..." + reset)
            break
    ...
    if repair:
        ...
        print(f"{yellow}[ Repair ]: Tunggu {repair} lagi..." + reset)
...
def repair_spinner(token):
    ...
    if response.status_code == 200:
        ...
        print(green + "[ Repair ]: Repair spinner sukses..." + reset)  # Warna hijau jika repair spinner sukses
    else:
        print(red + "[ Repair ]: Repair spinner gagal..." + reset)  # Warna merah jika repair spinner gagal
...
def check_requirement(token, id_tugas):
    ...
    try:
        ...
        if response.status_code == 500:
            print(red + f"[ Clear Task ]: Internal Server Error. Code {response}" + reset)
            return
        else:
            print(red + f"[ Clear Task ]: Failed to decode JSON {response}" + reset)
        return

    # ... existing code ...
    if response.status_code == 200:
        if result.get('message') == "it's impossible to do the requirement 2 times":
            print(yellow + "[ Clear Task ]: Already Claimed" + reset)  # Warna kuning jika task bisa dilakukan 2 kali

        elif not result.get('reward') and not result.get('success'):
            print(red + f"[ Clear Task ]: Tidak ada reward. {result.get('message')}" + reset)  # Warna merah jika reward gagal
        else:
            print(green + f"[ Clear Task ]: Sukses clear task | Reward {result.get('reward')} Poin" + reset)  # Warna hijau jika reward berhasil
    else:
        print(red + f"[ Clear Task ]: Failed to check requirement. {result.get('message')}")
...
def auto_upgrade(token, spinner_id):
    ...
    if response.status_code == 200:
        result = response.json()
        print(green + "[ Upgrade ]: " + result.get("message", "No message received") + reset)  # Warna hijau jika upgrade berhasil
    else:
        print(red + "[ Upgrade ]: Failed to upgrade spinner" + reset)  # Warna merah jika upgrade gagal
...
def def_rocket(token, spinner_id):
    ...
    if response.status_code == 200:
        result = response.json()
        print(green + "[ Rocket ]: Roket Berhasil diaktifkan!" + reset)  # Warna hijau jika roket aktif
    else:
        print(red + "[ Rocket ]: Tidak ada roket!" + reset)  # Warna merah jika gagal aktikan roket
...
def claim_daily(token):
    ...
    if response.status_code == 200:
        result = response.json()
        print(green + f"[ Claim Daily ]: Claim daily berhasil! Reward: {result.get('reward_text')}" + reset)  # Warna hijau jika claim daily berhasil
    else:
        print(red + "[ Claim Daily ]: Tidak ada claim daily" + reset)  # Warna merah jika claim daily gagal
...
def def_fullhp(token, spinner_id):
    ...
    if response.status_code == 200:
        result = response.json()
        print(green + "[ FullHP ]: " + result.get("message", "No message received") + reset)  # Warna hijau jika full hp berhasil
    elif response.status_code == 400:
        print(red + "[ FullHP ]: Full HP tidak ada" + reset)  # Warna merah jika full hp tidak ada
    else:
        print(red + "[ FullHP ]: Gagal mengaktifkan Full HP" + reset)  # Warna merah jika gagal mengaktifkan full hp

def main():
    banner = f"""
    {blue}Newbie Airdrop {white}SpinnerCoin Auto Claimer
    ...
    while True:
        ...
        for index, token in enumerate(tokens):
            print(f"{Fore.CYAN+Style.BRIGHT}============== [  Akun ke-{index + 1}  ] ==============" + reset)  # Mencetak nama akun
            data = get_data(token)
            if data:
                spinners = data.get('initData', {}).get('spinners', [  ])
                if spinners:
